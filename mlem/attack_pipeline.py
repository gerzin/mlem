...
from mlem.utilities import create_attack_dataset, save_pickle_bz2, save_txt, create_random_forest, \
    create_attack_dataset_from_lime_centroids, stat_sample_dataset, get_labels_distr, oversample
import time
...
def perform_attack_pipeline(
    ...
    """
    print(f"Attacking index {idx} using {attack_strategy.name} strategy with {num_shadow_models} shadow models")
    ...
    if local_attack_dataset is not None:
        ...
        if statistical_generation:
            x_attack = stat_sample_dataset(x_neigh, n_samples=6500)
            y_attack = local_model.predict(x_attack)
            d_0, d_1 = get_labels_distr(y_attack)
        ...
        print(f"[INFO] Using dataset generated by LIME | ", end='')
        ...
        d_0_bef, d_1_bef = get_labels_distr(y_attack)
        x_attack, y_attack = oversample(x_attack, y_attack, categorical_mask)
        d_0, d_1 = get_labels_distr(y_attack)
        print(
            f" label dis before overs {d_0_bef:.2f}%/{d_1_bef:.2f}% | after overs: {d_0:.2f}%/{d_1:.2f}% (nelems: {len(y_attack)})")
