...
class TrePanGenerator(Generator):
    ...
    def generate(self, sample, oracle=None, size=1000, budget=50000, **kwargs):
        ...
        if self.oracle_type == 'nn':
            with torch.no_grad():
                labels_bb = []
                train_set1 = MyDataset(sample)
                ...
                for inputs in train_loader:
                    ...
                    for i in temp:
                        labels_bb.append(i)
            balance_ratio = np.array(labels_bb).round().mean()
        else:
    ...
    def __generate_sample(self, records, distributions, types, populations, oracle, size, rules=None,
        ...
        if self.oracle_type == 'nn':
            train_set1 = MyDataset(jointly_estimated_samples)
            train_loader = DataLoader(dataset=train_set1,
                                      batch_size=32,
                                      shuffle=True,
                                      num_workers=2)
            ...
            with torch.no_grad():
                ...
                for inputs in train_loader:
                    ...
                    for i in temp:
                        labels_bb.append(i)
            oracle_predictions = np.array(labels_bb).round()
        else:
